...
const Submittask = ({navigation, route}) => {
  ...
  const handleMcqAnswerChange = (qNo, answer) => {
    console.log(
      '[Submittask] MCQ answer changed for Q:',
      qNo,
      'Answer:',
      answer,
    );
  ...
  const submitanswer = async () => {
    ...
    try {
      ...
      if (task.type === 'mcq') {
        ...
        console.log(
          '[Submittask] MCQ Payload:',
          JSON.stringify(payload, null, 2),
        );

        console.log(
          '[Submittask] Sending to:',
          `${API_URL}/api/Students/submit-quiz`,
        );
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              result.error || result.message || 'Quiz submission failed',
            );
          }
        ...
        console.log(
          '[Submittask] Sending to:',
          `${API_URL}/api/Students/submit-task-file`,
        );
        ...
        const responseText = await response.text();
        console.log(
          '[Submittask] Raw response (first 200 chars):',
        ...
        try {
          ...
          if (!response.ok) {
            console.error('[Submittask] Submission failed:', result);
            throw new Error(
              result.error || result.message || 'File submission failed',
            );
          }
          console.log('[Submittask] Submission successful:', result);
            ...
            <View style={styles.fileSection}>
              <View style={styles.fileInstructions}>
                ...
                <Text style={styles.instructionText}>
                  Upload your {task.type} document
                </Text>
