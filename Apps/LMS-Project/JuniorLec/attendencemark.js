...
const JAttendanceScreen = ({route, navigation}) => {
  ...
  console.log(
    'class data is ' + classData.class_type,
    classData?.teacher_offered_course_id,
    classData.fixed_date,
  );
  console.log('fd ' + fd);
  ...
  console.log(
    'Teacher Offered Course ID: ' + teacherOfferedCourseId + classData.venue,
  );
  useEffect(() => {
    if (classData) {
      setSectionInfo({
  ...
  const fetchStudentList = async () => {
    ...
    try {
      ...
      const studentsWithAttendance = adata.students.map(student => {
        ...
        const statusFromApi =
          student.attendance_status || student.attedance_status;

        // Convert API status to our app's status format
        let attendanceStatus;
        if (statusFromApi) {
          attendanceStatus =
            statusFromApi.toLowerCase() === 'p'
    ...
    <View style={styles.sectionContainer}>
      ...
      <View style={styles.actionButtonsRow}>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate('JAddseatingPlan', {
              classData,
              apiResponse,
