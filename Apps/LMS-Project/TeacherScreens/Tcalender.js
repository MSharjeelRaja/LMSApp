...
const Tcalender = ({ navigation }) => {
  ...
  const fetchCalendarData = async () => {
    try {
      const response = await fetch(`${API_URL}/api/Students/calendar`);
      ...
      // Add excluded days
      data.exculded_days.forEach(day => {
        allEvents.push({
          date: day.date,
          type: day.type,
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.currentDateContainer}>
        <Text style={styles.currentDateText}>
          Today is {dayNames[new Date().getDay()]}, {monthNames[new Date().getMonth()]} {new Date().getDate()}, {new Date().getFullYear()}
        </Text>
      </View>
